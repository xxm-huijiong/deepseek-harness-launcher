...
- **检查更新**：启动时自动对比本地 dsh 版本与官方源（npm `latest` 标签 + 官方仓库 `master`）；发现新版弹出**前置更新窗口**，一键自动更新（详见下方「更新 dsh」）。检查源按「先系统代理、失败再直连」依次尝试，代理/直连两种网络环境都能用
- **提示预发布更新**（默认开启）：勾选时除 npm `latest` 正式通道外，还跟踪 npm `next`/`alpha` 标签与官方仓库 `master`（可能比 `latest` 更新，如 `0.1.6-alpha.2`）；取消后只跟正式通道。启动时检查与手动「检查更新」都按此选项执行
- **任务提醒**：任务完成/失败/取消时气泡提醒（按"回合结束"判定，子代理会话不打扰）；勾选「后台才提醒」（默认开启）则**仅当启动器窗口不在前端时**才提醒
- **选项行**：`≡ 操作` 面板分两行——第一行按钮，第二行六个选项（启动时自动运行 / 启动时检查更新 / 提示预发布更新 / 任务提醒 / 后台才提醒 / 内置浏览器打开），托盘右键菜单中有对应勾选项
...
2. 执行 `npm install -g @deepseek-ai/dsh@<检测到的版本>` 更新 dsh 全局包（无需源码构建）
   - 按「检测到的那个版本」安装而非 `latest` 标签：勾选「提示预发布更新」时目标可能是仓库 `master` 上的 alpha（比 `latest` 新），装 `latest` 会退回旧版、导致每次启动重复提示同一新版
   - 若该版本还没发布到 npm（只在官方仓库里），会自动降级为安装 npm 上的最新发布版，并在日志中说明
3. 更新完成后重启服务即使用新版
